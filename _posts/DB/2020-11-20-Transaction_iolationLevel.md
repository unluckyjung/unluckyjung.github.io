...
- 한 트랜잭션에서 같은 쿼리를 두번 실행했을때, 수정된 데이터를 다르게 읽기 때문에 **다른결과가 나온다.**
- ACID중 **Consistency(일관성)** 을 지키지 못한다.
...
* 이때 트랜잭션이 직접적으로 변경하거나 접근하고 있는 데이터를 제외한 다른 데이터에는 접근이 가능하다. (Update, Insert)
* 또한 범위 자체에 대한 `Locking`은 걸리지 않는다.
...
- **삽입하지 않은놈이 갑자기 보이거나, 삭제하지도 않았는데 있던놈이 사라지는것이다.**
- 이 문제는 자신이 타켓 자료를 변경하지 않는 이상. 처음 조회한 그 형상 그대로를 유지한다는 특징때문에 발생한다. 
- 다른 트랜잭션에서 그 자료를 조작하고 커밋을 했다고 하더라도 말이다.
...
- 2번 트랜잭션이 끝나기전, 2번 보다 **늦게 시작한 3번 트랜잭션**에서 `1번 id`를 가진 데이터를 **삭제하고 커밋**까지 완료했다고 해보자.
